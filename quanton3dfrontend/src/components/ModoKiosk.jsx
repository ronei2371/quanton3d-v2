...
export default function ModoKiosk() {
  ...
  useEffect(() => {
    const iv = setInterval(() => {
      ...
      if (!paused) {
        setElapsed((e) => {
          const next = e + 1;
          const porCamada = totalSecs > 0 && params.totalCamadas > 0 ? totalSecs / params.totalCamadas : 1;
          setCamadaAtual(Math.min(params.totalCamadas, Math.floor(next / porCamada)));
          return next;
        });
      }
    }, 1000);
  ...
  }, [paused, params.totalCamadas, totalSecs]);

  const pct = params.totalCamadas > 0 ? Math.round((camadaAtual / params.totalCamadas) * 100) : 0;
  ...
  function normalizarDraft() {
    ...
    next.antiAliasing = draft.antiAliasing || params.antiAliasing;

    return next;
  }

  const salvar = () => {
    setParams(normalizarDraft());
    setEditing(false);
    setCamadaAtual(0);
    setElapsed(0);
    setPaused(false);
  ...
  return (
    <div style={s.kiosk}>
      ...
      <div style={s.progressWrap}>
        ...
        <div style={s.progressLayers}>Camada {camadaAtual} de {params.totalCamadas}</div>
      </div>
      ...
      <div style={s.ctrlRow}>
        ...
        <button
          style={{ ...s.btn, borderColor: "rgba(0,212,255,.35)", color: "#00d4ff" }}
          onClick={() => {
            setCamadaAtual(0);
            setElapsed(0);
            setPaused(false);
        ...
        <button
          style={{ ...s.btn, borderColor: "rgba(255,255,255,.15)", color: "rgba(255,255,255,.5)" }}
          onClick={() => {
            setDraft(params);
            setEditing(true);
          }}
        ...
        <div style={s.modalOverlay}>
          <div style={s.modal}>
            ...
            <div style={s.modalBtns}>
              <button style={{ ...s.btn, flex: 1 }} onClick={() => setEditing(false)}>
                Cancelar
              </button>
              <button
                style={{ ...s.btn, flex: 1, borderColor: "rgba(0,212,255,.4)", color: "#00d4ff" }}
                onClick={salvar}
              >
                Salvar
              </button>
...
function MetricCard({ label, value, unit, color }) {
  const colors = {
    cyan: { bg: "rgba(0,212,255,.07)", border: "rgba(0,212,255,.2)", val: "#00d4ff" },
    green: { bg: "rgba(0,255,136,.05)", border: "rgba(0,255,136,.15)", val: "#00ff88" },
