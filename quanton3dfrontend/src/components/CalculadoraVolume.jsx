...
export default function CalculadoraVolume() {
  ...
  return (
    <section style={S.section}>
      <div style={S.header}>
        <span style={S.badge}>⚡ CALCULADORA SIMPLES</span>
        <h2 style={S.title}>Custo Real de Impressão</h2>
        <p style={S.subtitle}>Selecione sua impressora e resina — os valores típicos são preenchidos automaticamente. Ajuste o que precisar e veja o custo na hora.</p>
      </div>
      ...
      <div style={S.formCard}>
        ...
        {/* IMPRESSORA — destaque */}
        <div style={{ background: "rgba(79,209,255,0.05)", border: "1px solid rgba(79,209,255,0.2)", borderRadius: "12px", padding: "14px 16px", marginBottom: "16px" }}>
          <p style={{ ...S.miniTitle, color: "#4fd1ff", marginBottom: "10px" }}>🖨️ Selecione sua impressora</p>
          <div style={S.grid2}>
            <div style={S.field}>
              <label style={S.label}>Modelo da impressora</label>
              <select value={impressoraSelecionada} onChange={e => handleImpressoraChange(e.target.value)} style={S.select}>
            ...
            <div style={S.field}>
              <label style={S.label}>Potência da máquina (W)</label>
              <div style={S.inputWrap}>
                <input type="number" value={potenciaW} min={0} step={1}
                  onChange={e => { setImpressoraSelecionada(""); setPotenciaW(Number(e.target.value)); }}
                  style={S.input} placeholder="Ex: 65" />
                <span style={S.suffix}>W</span>
              </div>
              {impressoraSelecionada && (
                <span style={{ fontSize: "11px", color: "#49e68b", marginTop: "3px" }}>
                  ✅ Preenchido automaticamente para {impressoraSelecionada}
                </span>
              )}
              {!impressoraSelecionada && (
                <span style={{ fontSize: "11px", color: "#9fb4c7", marginTop: "3px" }}>
                  Selecione a impressora acima para preencher automaticamente
                </span>
              )}
            </div>
          </div>
