...
const parametrosVazios = PARAMETRO_FIELDS.reduce((acc, field) => {
  acc[field.name] = "";
  return acc;
}, {});
...
function parametroResumo(parametros = {}) {
...
export default function GaleriaContent({ cliente }) {
  ...
  const [parametros, setParametros] = useState(parametrosVazios);
  ...
  async function enviar(event) {
    ...
    try {
      ...
      setParametros(parametrosVazios);
      setFoto(null);
      event.target.reset();
    } catch (err) {
      ...
      {aba === "enviar" ? (
        <form className="gallery-form" onSubmit={enviar}>
          <div className="form-grid gallery-form-grid">
            ...
            <label><span>Impressora</span><input value={form.impressora} onChange={(e) => setForm({ ...form, impressora: e.target.value })} placeholder="Ex.: Saturn 4 Ultra" /></label>
            <label className="full-width"><span>Foto da peça</span><input type="file" accept="image/*" onChange={(e) => setFoto(e.target.files?.[0] || null)} /></label>
          </div>
            ...
            <div className="form-grid gallery-settings-grid">
              {PARAMETRO_FIELDS.map((field) => (
                <label key={field.name}>
                  ...
                  <input value={parametros[field.name]} onChange={(e) => setParametros({ ...parametros, [field.name]: e.target.value })} placeholder={field.placeholder} />
                </label>
              ))}
            </div>
            <label className="gallery-observation"><span>Observações</span><textarea rows="3" value={form.observacao} onChange={(e) => setForm({ ...form, observacao: e.target.value })} placeholder="Conte detalhes importantes da impressão." /></label>
          </div>
        ...
        <div className="gallery-approved-list">
          ...
          {itens.map((item) => (
            <article className="gallery-approved-card" key={item._id}>
              {item.imagem && <img src={imagemSrc(item.imagem)} alt={`Peça impressa com ${item.resina || "resina"}`} />}
              <div>
                ...
                <div className="gallery-param-list">
                  {parametroResumo(item.parametros).map((parametro) => <span key={parametro.label}>{parametro.label}: {parametro.value}</span>)}
                </div>
              </div>
            </article>
          ))}
