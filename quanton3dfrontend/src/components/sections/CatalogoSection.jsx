...
const RESINAS = [
  ...
  { img: "/images/resinas/lowsmell.webp", url: "https://quanton3d.com.br/produtos/resina-quanton-low-smell/", nome: "Lowsmell", cat: "Uso Geral", desc: "Resina rígida com odor praticamente imperceptível. Cura rápida e excelente precisão para peças decorativas e protótipos. Mesmo com odor reduzido, mantenha ventilação adequada durante o uso.", specs: "Baixíssimo odor | Rígida", volumes: ["500g", "1kg", "5kg"], cores: ["Grey", "Skin", "Clear"] },
...
const CATEGORIAS = ["Todas", "Uso Geral", "Engenharia", "Action Figures", "Odontologia", "Fundição"];
...
function CatalogoSection() {
  ...
  const resinasFiltradas = catSel === "Todas" ? RESINAS : RESINAS.filter((res) => res.cat === catSel);
  const r = resinasFiltradas[resinaSel] || resinasFiltradas[0];
  ...
  return (
    <section className="q-card q-panel">
      ...
      <div style={{ marginTop: "8px" }}>
        <h3 style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "1rem", margin: "0 0 12px" }}>
          <FlaskConical size={16} /> Nossas Resinas — 14 Linhas Exclusivas
        </h3>

        {/* Filtros por categoria */}
        <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "10px" }}>
          {CATEGORIAS.map((cat) => {
            const count = cat === "Todas" ? RESINAS.length : RESINAS.filter((res) => res.cat === cat).length;
            const ativo = cat === catSel;
            return (
              <button key={cat} type="button" onClick={() => selecionarCategoria(cat)}
                className="q-badge"
                style={{
                  cursor: "pointer", border: "1px solid",
                  borderColor: ativo ? "var(--primary)" : "var(--border-soft)",
                  background: ativo ? "rgba(47,123,255,0.14)" : "rgba(255,255,255,0.03)",
                  color: ativo ? "var(--primary)" : "var(--text-muted)",
                  fontWeight: ativo ? 700 : 400,
                  fontSize: "0.8rem",
                }}>
                {cat} <span style={{ opacity: 0.65, fontSize: "0.75rem" }}>({count})</span>
              </button>
            );
          })}
        </div>

        {/* Seleção individual de resina (filtrada) */}
        <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "16px" }}>
          {resinasFiltradas.map((res, i) => (
            <button key={res.nome} type="button" onClick={() => setResinaSel(i)}
              className="q-badge"
              style={{
                cursor: "pointer", border: "1px solid",
                borderColor: i === resinaSel ? "var(--primary)" : "var(--border-soft)",
                background: i === resinaSel ? "rgba(47,123,255,0.14)" : "rgba(255,255,255,0.03)",
                color: i === resinaSel ? "var(--primary)" : "var(--text-muted)",
              }}>
              {res.nome}
            </button>
          ))}
