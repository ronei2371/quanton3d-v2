  ...
  return (
    <div className="modal-backdrop">
      <section className="site-modal">
        ...
        {type === "galeriaPublica" && <GaleriaContent cliente={cliente} modoInicial="ver" ocultarAbas />}
    ...
    <div className="field">
      ...
      <div
        ...
        style={{
          ...
          fontSize: valores.resultado === null ? "1rem" : "1.25rem",
          fontWeight: 950,
          lineHeight: 1.35,
        }}
      >
        {valores.erro || (valores.resultado === null ? "O resultado aparecerá aqui." : `Resultado: ${formatarMm(valores.resultado)}`)}
      </div>
    </div>
  );
}
...
function GaleriaContent({ cliente, modoInicial = "enviar", ocultarAbas = false }) {
  ...
  return (
    <div className="admin-gallery-panel">
      <div className="admin-gallery-toolbar">
        <label><span>Status</span><select value={filtros.status} onChange={(e) => alterarFiltro("status", e.target.value)}><option value="pendente">Pendentes</option><option value="aprovado">Aprovados</option><option value="recusado">Recusados</option><option value="todos">Todos</option></select></label>
        <label><span>Data inicial</span><input type="date" value={filtros.dataInicio} onChange={(e) => alterarFiltro("dataInicio", e.target.value)} /></label>
        <label><span>Data final</span><input type="date" value={filtros.dataFim} onChange={(e) => alterarFiltro("dataFim", e.target.value)} /></label>
        <button type="button" onClick={carregarItens} disabled={carregando}>{carregando ? "Carregando..." : "Atualizar"}</button>
